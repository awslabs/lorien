...
"""
The module of TVM auto_scheduler workload extraction from DNN models.
"""
import argparse
...
def extract_from_models(configs: argparse.Namespace):
    ...
    workloads: Set[AutoSchedulerWorkload] = set()
        ...
        for name, (mod, params) in mod_n_params:
            for target in configs.target:
                ...
                tasks, _ = auto_scheduler.extract_tasks(
                    ...
                    target=target,
                    params=params,
                    include_simple_tasks=configs.include_simple_tasks,
                )

                # Task to workload
                for task in tasks:
                    try:
                        workloads.add(AutoSchedulerWorkload.from_task(task))
...
@register_config_parser("top.generate.auto_scheduler")
def define_config_extract() -> argparse.ArgumentParser:
    ...
    """
    parser = create_config_parser("AutoSchedulerWorkload Generation by Model Extraction")
    ...
    parser.add_argument(
        "-o", "--output", default="auto_scheduler_workloads.yaml", help="The output file path"
    )
    parser.add_argument(
        "--include-simple-tasks",
        default=False,
        action="store_true",
        help="Whether to extract simple tasks (without complex operators)",
    )
